...
HWTEST_F(BTarballPosixTarballTest, b_tarball_posix_tarball_EmptyFile_0100, testing::ext::TestSize.Level0)
{
    ...
    try {
        ...
        int ret = BProcess::ExecuteCmd({"touch", filePath.c_str()});
        EXPECT_EQ(ret, 0);
        // 创建用于存放解包文件的目录
        string unpackDirName = "unpack";
        string unpackDirPath = backupRootDirPath + unpackDirName;
        ret = BProcess::ExecuteCmd({"mkdir", unpackDirPath.c_str()});
        EXPECT_EQ(ret, 0);
        // 对空文件进行打包
        ...
        tarball.Publish();
        // 对包文件进行解包
        ret = BProcess::ExecuteCmd({"tar", "-xvf", tarballPath.c_str(), "-C", unpackDirPath.c_str()});
        EXPECT_EQ(ret, 0);
        // 比较源文件和解包文件的差异
        string unpackFilePath = unpackDirPath + filePath;
        ret = BProcess::ExecuteCmd({"diff", filePath.c_str(), unpackFilePath.c_str()});
        EXPECT_EQ(ret, 0);
    } catch (...) {
        EXPECT_TRUE(false);
...
HWTEST_F(BTarballPosixTarballTest, b_tarball_posix_tarball_RegularFile_0100, testing::ext::TestSize.Level0)
{
    ...
    try {
        ...
        string filePath = backupRootDirPath + fileName;
        int ret = BProcess::ExecuteCmd({"dd", "if=/dev/urandom", ("of=" + filePath).c_str(), "bs=1000", "count=1"});
        EXPECT_EQ(ret, 0);
        // 创建用于存放解包文件的目录
        string unpackDirName = "unpack";
        string unpackDirPath = backupRootDirPath + unpackDirName;
...
HWTEST_F(BTarballPosixTarballTest, b_tarball_posix_tarball_EmptyDir_0100, testing::ext::TestSize.Level0)
{
    ...
    try {
        ...
        string filePath = backupRootDirPath + fileName;
        int ret = BProcess::ExecuteCmd({"mkdir", filePath.c_str()});
        EXPECT_EQ(ret, 0);
        // 创建用于存放解包文件的目录
        string unpackDirName = "unpack";
        string unpackDirPath = backupRootDirPath + unpackDirName;
...
HWTEST_F(BTarballPosixTarballTest, b_tarball_posix_tarball_SymbolicLink_0100, testing::ext::TestSize.Level0)
{
    ...
    try {
        ...
        ret = BProcess::ExecuteCmd({"ln", "-s", filePath.c_str(), linkPath.c_str()});
        EXPECT_EQ(ret, 0);
        // 创建用于存放解包文件的目录
        string unpackDirName = "unpack";
        string unpackDirPath = backupRootDirPath + unpackDirName;
        ...
        ret = BProcess::ExecuteCmd({"diff", linkPath.c_str(), unpackFilePath.c_str()});
        EXPECT_EQ(ret, 0);
    } catch (...) {
        EXPECT_TRUE(false);
        GTEST_LOG_(INFO) << "BTarballPosixTarballTest-an exception occurred by BTarballPosixTarball.";
...
HWTEST_F(BTarballPosixTarballTest, b_tarball_posix_tarball_SuperLargeFile_0100, testing::ext::TestSize.Level0)
{
    ...
    try {
        ...
        string filePath = backupRootDirPath + fileName;
        int ret = BProcess::ExecuteCmd({"dd", "if=/dev/urandom", ("of=" + filePath).c_str(), "bs=1M", "count=8200"});
        EXPECT_EQ(ret, 0);
        // 创建用于存放解包文件的目录
        string unpackDirName = "unpack";
        string unpackDirPath = backupRootDirPath + unpackDirName;
